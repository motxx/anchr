...
This directory holds the Anchr v0 Paid Request exchange and Nostr message-format
specifications that Customer, Provider, and Oracle implementations use to
...
| ------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Paid Request Exchange](paid-request-exchange.md) | Nostr-native, Cashu-settled v0 exchange: actors, request/offer/selection/proof/release/redeem links, Cashu Payment Lock, and redeem/refund safety rules. |
...
Topics that independent implementations need not share live alongside the code
that owns them.
...
| Target architecture / role boundaries              | [`docs/architecture.md`](../docs/architecture.md)                           | Design-level actor and layer decisions, not a protocol message.                                                      |
...
| TLSNotary application-layer hardening              | `packages/sdk/src/proofs/tlsn-*`                                            | Replay defence, ReDoS-safe conditions, freshness.                                                                    |
| C2PA image + GPS binding verification              | `packages/sdk/src/proofs/c2pa-validation.ts`                                | Content Credentials manifest signature validation and signed GPS distance policy.                                    |
| Application-specific proof verification            | `packages/sdk/src/proofs/`                                                  | SDK verifier helpers and policies that other implementations need not share.                                         |
...
For the broader placement rule that separates protocol requirements from
