...
if (!FROST_AVAILABLE) {
  console.warn(
    "[e2e] frost-signer binary not available — FROST tests will be skipped.",
  );
...
suite(
  "e2e: FROST 2-of-3 independent Oracle threshold (Anchr / CommunityOracle-A / CommunityOracle-B)",
  () => {
    // =========================================================================
    ...
    beforeAll(async () => {
      ...
      for (let i = 0; i < TOTAL; i++) {
        ...
        const identifier = (r.data!.secret_package as Record<string, unknown>)
      ...
      for (let i = 0; i < TOTAL; i++) {
        ...
        for (let j = 0; j < TOTAL; j++) {
          if (j === i) continue;
          othersMap[round1Results[j]!.identifier] = JSON.parse(
      ...
      for (let i = 0; i < TOTAL; i++) {
        ...
        for (let j = 0; j < TOTAL; j++) {
          if (j === i) continue;
          const pkgsFromJ = round2Results[j]!.packages as Record<
            string,
            unknown
          >;
          round2ForMe[round1Results[j]!.identifier] =
            pkgsFromJ[round1Results[i]!.identifier];
        }

        const othersR1Map: Record<string, unknown> = {};
        for (let j = 0; j < TOTAL; j++) {
          if (j === i) continue;
          othersR1Map[round1Results[j]!.identifier] = JSON.parse(
            round1Results[j]!.package,
          );
        }
    ...
    test("Sign 2-of-3: Anchr + CommunityOracle-A → valid BIP-340 signature", async () => {
      const { signature } = await signWithPair(
        oracles[0]!,
        oracles[1]!,
        "cafebabe01",
      );
    ...
    test("all Oracle pairs on same message produce distinct, valid signatures", async () => {
      ...
      const { signature: sig1 } = await signWithPair(
        oracles[0]!,
        oracles[1]!,
        message,
      );
      const { signature: sig2 } = await signWithPair(
        oracles[0]!,
        oracles[2]!,
        message,
      );
      const { signature: sig3 } = await signWithPair(
        oracles[1]!,
        oracles[2]!,
        message,
      );
    ...
    test("Sign 3-of-3: all Oracle operators participate → valid signature", async () => {
      ...
      const r1s = await Promise.all(
        oracles.map((o) => signRound1(o.keyPackage)),
      );
      ...
      const r2s = await Promise.all(
        oracles.map((o, i) =>
          signRound2(
            o.keyPackage,
            JSON.stringify(r1s[i]!.data!.nonces),
            commitmentsJson,
            message,
          )
        ),
      ...
      const ver = await verifySignature(
        groupPubkey,
        agg.data!.signature as string,
        message,
      );
      expect(ver.ok).toBe(true);
      expect(ver.data!.valid).toBe(true);
...
suite(
  "e2e: FROST Oracle HTTP signer endpoints (nonce_id + mandatory verification)",
  () => {
