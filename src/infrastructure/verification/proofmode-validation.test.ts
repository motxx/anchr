...
/**
 * Build a minimal JPEG buffer (smallest valid JPEG: SOI + EOI markers).
 */
 ...
 * Returns a zip Buffer, or null if `zip` CLI is unavailable.
 */
function createProofModeZip(opts?: {
  ...
  try {
    ...
    return null; // zip CLI not available
  } finally {
...
describe("parseProofModeZip", () => {
  test("parses valid ProofMode zip with correct hash", async () => {
    ...
    if (!zip) return; // skip if zip unavailable
