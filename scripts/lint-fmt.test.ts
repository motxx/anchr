...
test("checks TypeScript, JavaScript, and Deno config files", () => {
  ...
  expect(shouldCheckFormat("example/app/src/view.tsx")).toBe(true);
...
test("excludes Markdown docs and non-Deno JSON artifacts", () => {
  ...
  expect(shouldCheckFormat("example/app/package-lock.json")).toBe(false);
});

test("excludes generated or shared-agent directories", () => {
  expect(shouldCheckFormat("example/expo-worker-app/src/app.ts")).toBe(false);
