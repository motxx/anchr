...
// Soft rules apply only outside this list (tests + scripts).
const SOFT_EXEMPT = [
  ".test.ts",
  ".test.tsx",
  "/test/",
...
async function main() {
  ...
  for (const v of printed) {
    console.error(
      `${v.severity.toUpperCase()} [${v.code}] ${v.file}:${v.line}:${v.col} — ${v.excerpt}`,
    );
  }
  ...
  console.log(
    `✓ type-safety lint passed (errors=0, warnings=${warns.length}).`,
  );
}
