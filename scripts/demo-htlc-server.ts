...
async function runHtlcDemo(emit: Emit): Promise<void> {
  ...
  if (relayOk) emitOk("system", `Nostr relay at ${RELAY_URL}`);
    ...
    emitFail("system", `Relay not reachable at ${RELAY_URL}`);
    return;
  }

  if (blossomOk) emitOk("system", `Blossom server at ${BLOSSOM_URL}`);
  else {
    emitFail("system", `Blossom not reachable at ${BLOSSOM_URL}`);
    return;
  }

  if (cashuOk) emitOk("system", `Cashu mint at ${CASHU_MINT_URL}`);
  else {
    emitFail("system", `Cashu mint not reachable at ${CASHU_MINT_URL}`);
    return;
  }
  ...
  emitOk("requester", "Identity created", {
    pubkey: requester.publicKey.slice(0, 16) + "...",
  });
  emitOk("worker", "Identity created", {
    pubkey: worker.publicKey.slice(0, 16) + "...",
  });
  emitOk("oracle", "Identity created", {
    pubkey: oracle.publicKey.slice(0, 16) + "...",
  });
  ...
  emitOk("oracle", "Preimage generated", {
    hash: preimageEntry.hash.slice(0, 16) + "...",
  });
  ...
  emitOk("requester", `Minted ${holdAmountSats} sats`, {
    proofs: bountyResult.proofs.length,
  });
  emitInfo(
    "requester",
    "Phase 1: no HTLC conditions, Requester holds bearer tokens",
  );
  ...
  if (pubResult.successes.length > 0) {
    emitOk("requester", "Published to relay", {
      event_id: queryEvent.id.slice(0, 16) + "...",
    });
  } else {
  ...
  emitStep(
    "requester",
    "Announcing Worker selection (kind 7000 status=processing)...",
  );
  ...
  if (selPubResult.successes.length > 0) {
    emitOk("requester", "Selection announced", {
      event_id: selectionEvent.id.slice(0, 16) + "...",
    });
  } else {
    ...
    try {
      ...
      if (p.status === "processing") {
        ...
        if (sel.selected_worker_pubkey === worker.publicKey) {
          ...
          emitInfo(
            "worker",
            `HTLC token received (${sel.htlc_token?.slice(0, 20)}...)`,
          );
          break;
        }
      }
  ...
  emitStep(
    "worker",
    "Creating data, encrypting (AES-256-GCM), uploading to Blossom...",
  );
  ...
  emitOk("worker", `Uploaded to Blossom`, {
    hash: uploadResult.blossom.hash.slice(0, 16) + "...",
  });
  ...
  emitOk(
    "worker",
    "K encrypted to K_R (Requester) and K_O (Oracle) via NIP-44",
  );
  ...
  if (resPubResult.successes.length > 0) {
    emitOk("worker", "Result published", {
      event_id: responseEvent.id.slice(0, 16) + "...",
    });
  } else {
  ...
  emitOk("requester", `Result decrypted`, {
    nonce_echo: parsedResponse.nonce_echo,
  });
  ...
  if (dmPubResult.successes.length > 0) {
    emitOk("oracle", "Preimage delivered via NIP-44 DM", {
      event_id: preimageEvent.id.slice(0, 16) + "...",
    });
  } else {
  ...
  if (dmPayload.type === "preimage" && dmPayload.query_id === queryId) {
    emitOk("worker", `Preimage received`, {
      preimage: dmPayload.preimage.slice(0, 16) + "...",
    });
  ...
  if (redeemResult) {
    emitOk("worker", `HTLC redeemed: ${redeemResult.amountSats} sats`, {
      proofs: redeemResult.proofs.length,
    });
  } else {
    emitWarn(
      "worker",
    ...
    if (redeemInspect && redeemInspect.amountSats > 0) {
      ...
      emitOk(
        "worker",
        "Worker has preimage + private key — both conditions satisfied",
      );
    } else {
...
console.log(
  `[demo-server] Anchr HTLC Demo UI running at http://localhost:${PORT}`,
);
