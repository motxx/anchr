/**
 * Full E2E: Seller (Requester) → Buyer (Worker) via Anchr + Stripe + TLSNotary + HTLC Escrow
 *
 * Flow (Worker-first):
 * 1. Seller: POST /hash → get preimage hash from Oracle
 * 2. Seller: Create Stripe Payment Link (Product → Price → Link)
 ...
 * 6. Buyer:  Browser pays via Payment Link (test card)
 * 7. Buyer:  Get PaymentIntent ID from Stripe API
...
// Stripe Payment browser — bottom left (blank until payment step)
...
const productResp = await fetch(`${STRIPE_API}/v1/products`, {
  ...
  body: "name=BTC+Swap+¥100&type=service",
});
const productData = await productResp.json();
const PRODUCT_ID = productData.id;
console.log(`[${elapsed()}] Product: ${PRODUCT_ID}`);

// Create a Price for the product
const priceResp = await fetch(`${STRIPE_API}/v1/prices`, {
  method: "POST",
  headers: stripeHeaders,
  body: `product=${PRODUCT_ID}&unit_amount=100&currency=jpy`,
});
const priceData = await priceResp.json();
const PRICE_ID = priceData.id;
console.log(`[${elapsed()}] Price: ${PRICE_ID}`);
...
const PAYMENT_LINK_ID = linkData.id;
...
// TLSNotary target: Stripe PaymentIntents list (most recent).
// Conditions check for "succeeded" status.
const queryResp = await fetch(`${ANCHR_URL}/queries`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    description: `Prove Stripe payment (¥100 BTC Swap) — Payment Link ready`,
    verification_requirements: ["tlsn"],
    ttl_seconds: 600,
    tlsn_requirements: {
      target_url: "https://api.stripe.com/v1/payment_intents?limit=1",
      domain_hint: "api.stripe.com",
      conditions: [
        {
          type: "contains",
          expression: '"status": "succeeded"',
          description: "PaymentIntent must have status=succeeded",
        },
      ],
...
// Fill test card details — Stripe Checkout has direct inputs on the main page
console.log(`[${elapsed()}] Filling test card details...`);
...
// and fall back to normal card payment form
await page.waitForTimeout(2000);
await page.screenshot({ path: "/tmp/stripe-step2b.png" });
const linkCancelBtn = page.locator('button:has-text("キャンセル")').first();
if (await linkCancelBtn.count() > 0) {
  await linkCancelBtn.click();
  console.log(`[${elapsed()}] Link overlay dismissed`);
...
await page.screenshot({ path: "/tmp/stripe-step3.png" });

await flow(5, "complete", "Payment done");
console.log(`[${elapsed()}] Payment completed in browser`);

// ============================================================
// Step 7: Get PaymentIntent ID from Stripe API
// ============================================================
await flow(6, "active", "Getting PaymentIntent...");
console.log(`\n[${elapsed()}] === Step 7: Get PaymentIntent ID ===`);
...
// Prove the specific PaymentIntent endpoint
// Note: Stripe uses RSA certificates — MPC-TLS is slower than ECDSA sites.
...
await requesterPage.screenshot({ path: "/tmp/e2e-stripe-requester-ui.png" });
await workerPage.screenshot({ path: "/tmp/e2e-stripe-worker-ui.png" });
