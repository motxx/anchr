...
import type { TlsnAttestation, TlsnRequirement } from "@anchr/tlsn-toolkit/tlsn-types";
import type {
  ClaimProof,
...
export interface CreateProofGateCampaignInput<C extends ProofGateCondition = ProofGateCondition> {
  id: string;
  name: string;
  conditions: C[];
...
export interface ProofGateServiceOptions<C extends ProofGateCondition = ProofGateCondition> {
  store: ProofGateStore<C>;
  nullifierSecret: string;
  identityPathForCondition: (condition: C) => string | undefined;
  validateTlsnFn?: ValidateTlsnFn;
  conditionEvaluator?: (condition: C, verifiedData: VerifiedProofData) => ConditionResult<C>;
...
export interface SubmitProofGateClaimResult<C extends ProofGateCondition = ProofGateCondition>
...
export class ProofGateService<C extends ProofGateCondition = ProofGateCondition> {
  ...
  readonly #evaluateCondition: (condition: C, verifiedData: VerifiedProofData) => ConditionResult<C>;
  ...
  constructor(opts: ProofGateServiceOptions<C>) {
    ...
    this.#evaluateCondition = opts.conditionEvaluator ??
      ((condition, data) => evaluateProofGateCondition(condition, data, opts.accountAgeConditionTypes));
    this.#settlementProvider = opts.settlementProvider;
    this.#maxAttestationAgeSeconds = opts.maxAttestationAgeSeconds ?? 600;
  }

  async createCampaign(input: CreateProofGateCampaignInput<C>): Promise<ProofGateCampaign<C>> {
    ...
    if (!input.conditions.some((c) => this.#identityPathForCondition(c))) {
      throw new ProofGateError(400, "missing_identity_condition", "At least one condition must expose a stable account identity path");
    }
  ...
  async reserveClaim(campaignId: string, claimantPubkey: string): Promise<ProofGateClaim<C>> {
    ...
    if (!isHexPubkey(claimantPubkey)) {
      throw new ProofGateError(400, "invalid_claimant_pubkey", "claimant_pubkey must be a 32-byte or compressed secp256k1 hex public key");
    }
    const now = Math.floor(Date.now() / 1000);
    ...
    if (claim.settlement.status === "failed") {
      throw new ProofGateError(502, "settlement_reserve_failed", claim.settlement.error ?? "Failed to reserve claim settlement");
    }
    await this.#store.createClaim(claim);
    return { ...claim, preimage: "" };
  }

  async submitClaim(campaignId: string, input: SubmitProofGateClaimInput): Promise<SubmitProofGateClaimResult<C>> {
    const campaign = await this.#requireLiveCampaign(campaignId);
    await this.#ensureBudgetAvailable(campaign);
    ...
    if (input.proofs.length !== campaign.conditions.length) {
      throw new ProofGateError(400, "proof_count_mismatch", `Expected ${campaign.conditions.length} proofs, got ${input.proofs.length}`);
    }
    ...
    const replayOk = await this.#store.reservePresentationHashes(campaign.id, claim.id, hashes);
    if (!replayOk) {
      await this.#rejectClaim(campaign, claim, "TLSNotary presentation already used");
      throw new ProofGateError(409, "presentation_replay", "One or more TLSNotary presentations were already submitted");
    }
    ...
    try {
      ...
      await this.#rejectClaim(campaign, claim, err instanceof Error ? err.message : String(err));
      throw err;
    }

    const nullifierHash = deriveNullifier(campaign, verifiedProofs, this.#nullifierSecret, this.#identityPathForCondition);
    if (!nullifierHash) {
      const reason = "No stable account identity could be extracted from verified proof data";
      await this.#rejectClaim(campaign, claim, reason);
      throw new ProofGateError(400, "missing_nullifier", reason);
    }
    const duplicate = await this.#store.findApprovedByNullifier(campaign.id, nullifierHash);
    if (duplicate) {
    ...
    const result = verifyProofGateClaim(campaign, verifiedProofs, preimage, this.#evaluateCondition);
    claim.status = result.all_passed ? "approved" : "rejected";
    claim.nullifier_hash = nullifierHash;
    claim.results = result.results;
    claim.failures = result.failures;
    if (result.all_passed) {
      claim.settlement = await this.#releaseSettlement(campaign, claim, preimage);
      result.settlement = claim.settlement;
    } else {
      await this.#rejectSettlement(campaign, claim, result.failures.join("; ") || "claim verification failed");
    }
  ...
  async #requireLiveCampaign(campaignId: string): Promise<ProofGateCampaign<C>> {
    const campaign = await this.#store.getCampaign(campaignId);
    if (!campaign) throw new ProofGateError(404, "campaign_not_found", "Campaign not found");
    if (campaign.status !== "live") {
      throw new ProofGateError(409, "campaign_not_live", `Campaign is ${campaign.status}`);
    }
    return campaign;
  }
  ...
  async #ensureBudgetAvailable(campaign: ProofGateCampaign<C>): Promise<void> {
    ...
    if (approved >= maxProofGateClaims(campaign)) {
      throw new ProofGateError(409, "budget_exhausted", "Campaign budget is exhausted");
    }
  }
  ...
  async #getOrCreateReservedClaim(
    ...
    if (input.claim_id) {
      ...
      if (!claim || claim.campaign_id !== campaignId) {
        throw new ProofGateError(404, "claim_not_found", "Reserved claim not found");
      }
      if (claim.status !== "reserved") {
        throw new ProofGateError(409, "claim_not_reserved", `Claim is ${claim.status}`);
      }
      if (claim.claimant_pubkey.toLowerCase() !== input.claimant_pubkey.toLowerCase()) {
        throw new ProofGateError(409, "claimant_pubkey_mismatch", "Reserved claim belongs to a different claimant_pubkey");
      }
      return claim;
    }
  ...
  async #releaseSettlement(
    ...
    return this.#settlementProvider.releaseClaimSettlement({ campaign, claim, preimage });
  }
  ...
  async #rejectSettlement(
    ...
    await this.#settlementProvider?.rejectClaimSettlement?.({ campaign, claim, reason });
  }
  ...
  async #verifyPresentations(
    ...
    const requirements = toProofGateTlsnRequirements(campaign.conditions, this.#maxAttestationAgeSeconds);
    const verified = new Map<number, VerifiedProofData>();
    const seenIndexes = new Set<number>();

    for (const proof of proofs) {
      if (!Number.isInteger(proof.condition_index) || proof.condition_index < 0 || proof.condition_index >= campaign.conditions.length) {
        throw new ProofGateError(400, "invalid_condition_index", `Invalid condition_index ${proof.condition_index}`);
      }
      if (seenIndexes.has(proof.condition_index)) {
        throw new ProofGateError(400, "duplicate_condition_index", `Duplicate condition_index ${proof.condition_index}`);
      }
      seenIndexes.add(proof.condition_index);

      const result = await this.#validateTlsn({ presentation: proof.presentation }, requirements[proof.condition_index]!);
      const failed = !result.available || !result.signatureValid || !result.serverIdentityValid || !result.attestationFresh ||
        result.conditionResults.some((r) => !r.passed) || !result.verifiedData;
      if (failed) {
        throw new ProofGateError(422, "tlsn_verification_failed", result.failures.join("; ") || "TLSNotary proof failed verification");
      }
...
export function generateProofGateClaimHash(): { preimage: string; hash: string } {
...
export function maxProofGateClaims(criteria: { token_amount_per_claim: number; total_budget_sats: number }): number {
...
export function evaluateProofGateCondition<C extends ProofGateCondition>(
  ...
  if (!expectedHost) {
    return { condition, passed: false, reason: `Invalid target URL: ${condition.target_url}` };
  }
  ...
  try {
    ...
    return { condition, passed: false, reason: "Response body is not valid JSON" };
  }

  const rawValue = resolveDotPath(parsed, condition.jsonpath);
  if (rawValue === undefined) {
    return { condition, passed: false, reason: `JSONPath "${condition.jsonpath}" not found in response` };
  }
...
export function verifyProofGateClaim<C extends ProofGateCondition>(
  ...
  preimage: string,
  evaluateCondition: (condition: C, verifiedData: VerifiedProofData) => ConditionResult<C> = evaluateProofGateCondition,
...
export function deriveProofGateNullifier<C extends ProofGateCondition>(
  ...
  return deriveNullifier(campaign, verifiedProofs, secret, identityPathForCondition);
}
...
function validateProofGateCampaign(input: CreateProofGateCampaignInput): string[] {
  const errors: string[] = [];
  if (!input.id?.trim()) errors.push("id is required");
  if (!input.name?.trim()) errors.push("name is required");
  if (!input.conditions?.length) errors.push("at least one proof condition is required");
  for (const [i, condition] of input.conditions.entries()) {
    if (!condition.type?.trim()) errors.push(`conditions[${i}].type is required`);
    if (!condition.target_url?.trim()) errors.push(`conditions[${i}].target_url is required`);
    else {
      try {
    ...
    if (!condition.jsonpath?.trim()) errors.push(`conditions[${i}].jsonpath is required`);
    if (!condition.description?.trim()) errors.push(`conditions[${i}].description is required`);
    if (condition.min_value !== undefined && condition.min_value <= 0) {
      errors.push(`conditions[${i}].min_value must be positive`);
    }
  }
  if (input.token_amount_per_claim <= 0) errors.push("token_amount_per_claim must be positive");
  if (input.total_budget_sats <= 0) errors.push("total_budget_sats must be positive");
  if (input.token_amount_per_claim > 0 && input.total_budget_sats > 0 && input.total_budget_sats < input.token_amount_per_claim) {
...
function isHexPubkey(value: string): boolean {
  return /^[0-9a-fA-F]{64}$/.test(value) || /^(02|03)[0-9a-fA-F]{64}$/.test(value);
}
