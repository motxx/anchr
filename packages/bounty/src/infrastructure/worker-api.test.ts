...
function makeTestAppWithOracle(
  opts: { pass?: boolean; oracleId?: string } = {},
) {
...
describe("buildWorkerApiApp with injected deps", () => {
  ...
  test(
    "POST /queries succeeds without htlc field",
    withOpenAuth(async () => {
      const { app } = makeTestApp();
      const res = await app.request("http://localhost/queries", {
        method: "POST",
  ...
  test(
    "POST /queries creates an HTLC query via injected service",
    withOpenAuth(async () => {
  ...
  test(
    "GET /queries/:id returns query detail",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
  ...
  test(
    "POST /queries/:id/cancel cancels via injected service",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      const query = queryService.createQuery({ description: "Test query" });
      const res = await app.request(
        `http://localhost/queries/${query.id}/cancel`,
        {
          method: "POST",
        },
      );
  ...
  test(
    "GET /queries lists only open queries from injected service",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
  ...
  test(
    "isolated app instances do not share state",
    withOpenAuth(async () => {
...
describe("HTLC endpoints", () => {
  ...
  test(
    "POST /queries creates HTLC query when htlc provided",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      const res = await app.request("http://localhost/queries", {
        method: "POST",
  ...
  test(
    "GET /queries/:id/quotes returns quotes",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      const query = queryService.createQuery({ description: "HTLC" }, {
        escrow: escrowInfo,
      });
      queryService.recordQuote(query.id, {
        worker_pubkey: "w1",
        quote_event_id: "e1",
      ...
      const res = await app.request(
        `http://localhost/queries/${query.id}/quotes`,
      );
      expect(res.status).toBe(200);
  ...
  test(
    "POST /queries/:id/quotes records a quote",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      const query = queryService.createQuery({ description: "HTLC" }, {
        escrow: escrowInfo,
  ...
  test(
    "POST /queries/:id/select selects worker",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      const query = queryService.createQuery({ description: "HTLC" }, {
        escrow: escrowInfo,
      ...
      const res = await app.request(
        `http://localhost/queries/${query.id}/select`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
  ...
  test(
    "POST /queries/:id/result for HTLC does inline verification",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      const query = queryService.createQuery({ description: "HTLC" }, {
        escrow: escrowInfo,
      ...
      const res = await app.request(
        `http://localhost/queries/${query.id}/result`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
  ...
  test(
    "GET /queries/:id includes HTLC info",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      const query = queryService.createQuery({ description: "HTLC" }, {
        escrow: escrowInfo,
  ...
  test(
    "HTLC full lifecycle via HTTP (inline verification)",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      ...
      const createRes = await app.request("http://localhost/queries", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          description: "Full HTLC lifecycle",
          escrow: escrowInfo,
          oracle_ids: ["test-oracle"],
        }),
      });
...
describe("P2PK+FROST escrow endpoints", () => {
  ...
  test(
    "POST /queries accepts a p2pk_frost escrow",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      const res = await app.request("http://localhost/queries", {
        method: "POST",
  ...
  test(
    "GET /queries/:id round-trips the p2pk_frost discriminator",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestApp();
      const query = queryService.createQuery({ description: "FROST detail" }, {
        escrow: frostEscrow,
      });
      const res = await app.request(`http://localhost/queries/${query.id}`);
      expect(res.status).toBe(200);
  ...
  test(
    "POST /queries rejects a p2pk_frost escrow missing group_pubkey",
    withOpenAuth(async () => {
      const { app } = makeTestApp();
      const res = await app.request("http://localhost/queries", {
        method: "POST",
  ...
  test(
    "POST /queries rejects an htlc escrow missing hash",
    withOpenAuth(async () => {
      const { app } = makeTestApp();
      const res = await app.request("http://localhost/queries", {
        method: "POST",
...
describe("POST /hash", () => {
  ...
  test(
    ...
    withOpenAuth(async () => {
      const { app } = makeTestAppWithPreimage();
      const res1 = await app.request("http://localhost/hash", {
        method: "POST",
      });
      const res2 = await app.request("http://localhost/hash", {
        method: "POST",
      });
...
describe("HTLC inline verification with preimage", () => {
  ...
  test(
    "POST /queries/:id/result returns preimage for HTLC on success",
    withOpenAuth(async () => {
  ...
  test(
    "full HTLC lifecycle with POST /hash",
    withOpenAuth(async () => {
      const { app, preimageStore } = makeTestAppWithPreimage();

      // 1. Generate hash via API
      const hashRes = await app.request("http://localhost/hash", {
        method: "POST",
      });
...
describe("Quorum via HTTP", () => {
  test(
    "POST /queries creates query with quorum config",
    withOpenAuth(async () => {
      const store = createQueryStore();
      const registry = createOracleRegistry({ skipBuiltIn: true });
  ...
  test(
    "GET /queries/:id exposes quorum and attestations",
    withOpenAuth(async () => {
      const store = createQueryStore();
      const registry = createOracleRegistry({ skipBuiltIn: true });
      registry.register(makeMockOracle("oracle-a"));
      ...
      const json = await res.json() as {
        ...
        attestations: Array<{ oracle_id: string; passed: boolean }>;
      };
...
describe("End-to-end HTTP integration", () => {
  ...
  test(
    "POST /queries/:id/result with rejecting oracle → 400",
    withOpenAuth(async () => {
  ...
  test(
    "POST /queries/:id/result on expired query → fails",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestAppWithOracle();
      const query = queryService.createQuery({ description: "Quick expiry" }, {
        ttlMs: -1,
      });
  ...
  test(
    "POST /queries/:id/cancel on already-approved → fails",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestAppWithOracle();
      const query = queryService.createQuery({
        description: "Approve then cancel",
      }, { oracleIds: ["test-oracle"] });
      await queryService.submitQueryResult(query.id, {
        attachments: [],
        notes: "",
      }, { executor_type: "human", channel: "worker_api" });
  ...
  test(
    "double submit to the same query → second call fails",
    withOpenAuth(async () => {
      const { app, queryService } = makeTestAppWithOracle();
      const query = queryService.createQuery({ description: "Double submit" }, {
        oracleIds: ["test-oracle"],
      });
      const submit = () =>
        app.request(`http://localhost/queries/${query.id}/result`, {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ worker_pubkey: "w", attachments: [] }),
        });
  ...
  test(
    "POST /queries/:id/result on non-existent query → fails",
    withOpenAuth(async () => {
      const { app } = makeTestAppWithOracle();
  ...
  test(
    "POST /queries supports verification_requirements + nonce challenge",
    withOpenAuth(async () => {
      const { app } = makeTestAppWithOracle();
      const res = await app.request("http://localhost/queries", {
        method: "POST",
  ...
  test(
    "POST /queries supports expected_gps + max_gps_distance_km",
    withOpenAuth(async () => {
      const { app } = makeTestAppWithOracle();
      const res = await app.request("http://localhost/queries", {
        method: "POST",
  ...
  test(
    "POST /queries supports bounty payload",
    withOpenAuth(async () => {
      const { app } = makeTestAppWithOracle();
      const res = await app.request("http://localhost/queries", {
        method: "POST",
