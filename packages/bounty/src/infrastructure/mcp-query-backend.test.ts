...
describe("MCP query backend — default backend", () => {
  ...
  test("getQueryStatus returns status payload for existing query", async () => {
    ...
    const status = await backend.getQueryStatus(
      created.query_id as string,
    ) as Record<string, unknown>;
  ...
  test("getQueryStatus returns error for unknown query", async () => {
    ...
    const result = await backend.getQueryStatus("nonexistent") as Record<
      string,
      unknown
    >;
  ...
  test("listAvailableQueries returns open queries", async () => {
    ...
    await backend.createQuery({ description: "Q1" }, 600, {
      requester_type: "agent",
    });
    await backend.createQuery({ description: "Q2" }, 600, {
      requester_type: "agent",
    });

    const list = await backend.listAvailableQueries() as Array<
  ...
  test("cancelQuery cancels existing query", async () => {
    ...
    const result = await backend.cancelQuery(
      created.query_id as string,
    ) as Record<string, unknown>;
  ...
  test("getQueryAttachment returns error when no attachments", async () => {
    ...
    const result = await backend.getQueryAttachment(
      created.query_id as string,
      0,
    ) as Record<string, unknown>;
