...
async function isBlossomReachable(): Promise<boolean> {
  ...
  try {
    const res = await fetch(getBlossomConfig()!.serverUrls[0]!, {
...
describe("worker api photo proof (Blossom)", () => {
  test(
    "supports photo upload, submission, and attachment metadata",
    withEnv({ HTTP_API_KEY: undefined, HTTP_API_KEYS: undefined }, async () => {
      if (!(await isBlossomReachable())) {
        if (REQUIRE_INFRA) {
          const reason = isBlossomEnabled()
            ? "Blossom server not reachable"
            : "BLOSSOM_SERVERS must be set";
          throw new Error(
            `${reason}. Run: docker compose up -d blossom && export BLOSSOM_SERVERS=http://localhost:3333`,
          );
        }
        console.log(
          "[blossom-test] SKIPPED — Blossom server not reachable",
        );
        return;
      }
      ...
      const uploadResponse = await app.request(
        `http://localhost/queries/${query.id}/upload`,
        {
          method: "POST",
          body: form,
        },
      ...
      const detailResponse = await app.request(
        `http://localhost/queries/${query.id}`,
      );
      ...
      const metaResponse = await app.request(
        `http://localhost/queries/${query.id}/attachments/0/meta`,
      );
      ...
      expect(metaJson.access.preview_url).toContain(
        `/queries/${query.id}/attachments/0/preview`,
      );
      expect(metaJson.access.view_url).toContain(
        `/queries/${query.id}/attachments/0`,
      );
      expect(metaJson.access.meta_url).toContain(
        `/queries/${query.id}/attachments/0/meta`,
      );
      ...
      const viewResponse = await app.request(
        `http://localhost/queries/${query.id}/attachments/0`,
      );
...
test(
  "worker api creates queries over HTTP and enforces write API keys",
  withEnv(
    { HTTP_API_KEY: "secret-write-key", HTTP_API_KEYS: undefined },
    async () => {
      const app = buildWorkerApiApp({ queryService: testService });
      ...
      expect(createJson.description).toBe(
        "Check if Authorized Test Store is open",
      );
      expect(createJson.status).toBe("pending");
      expect(createJson.requester_meta?.requester_type).toBe("app");
      expect(createJson.requester_meta?.requester_id).toBe(
        "integration-test-client",
      );
      expect(createJson.query_api_url).toContain(
        `/queries/${createJson.query_id}`,
      );

      const storedQuery = testService.getQuery(createJson.query_id);
      expect(storedQuery?.requester_meta?.requester_type).toBe("app");
      expect(storedQuery?.requester_meta?.client_name).toBe(
...
describe("writeAuth middleware", () => {
  const authEnv = {
    ...
    HTTP_API_KEYS: undefined as string | undefined,
  };

  test(
    "rejects unauthenticated upload",
    withEnv(authEnv, async () => {
      const app = buildWorkerApiApp({ queryService: testService });
      const query = testService.createQuery({ description: "auth test" }, {
        ttlSeconds: 300,
      });
      const form = new FormData();
      form.append(
  ...
  test(
    "rejects unauthenticated result submission",
    withEnv(authEnv, async () => {
      const app = buildWorkerApiApp({ queryService: testService });
      const query = testService.createQuery({ description: "auth test" }, {
        ttlSeconds: 300,
  ...
  test(
    "rejects unauthenticated cancel",
    withEnv(authEnv, async () => {
      const app = buildWorkerApiApp({ queryService: testService });
      const query = testService.createQuery({ description: "auth test" }, {
        ttlSeconds: 300,
  ...
  test(
    "accepts X-API-Key header on cancel",
    withEnv(authEnv, async () => {
      const app = buildWorkerApiApp({ queryService: testService });
      const query = testService.createQuery({ description: "cancel test" }, {
        ttlSeconds: 300,
      });
      const res = await app.request(
        `http://localhost/queries/${query.id}/cancel`,
  ...
  test(
    "supports multiple API keys via HTTP_API_KEYS",
    withEnv(
      {
        HTTP_API_KEY: undefined as string | undefined,
        HTTP_API_KEYS: "alpha,bravo,charlie",
      },
      async () => {
        const app = buildWorkerApiApp({ queryService: testService });
...
describe("POST /queries validation", () => {
  ...
  test(
    "rejects missing description field",
    withEnv(openEnv, async () => {
      const app = buildWorkerApiApp({ queryService: testService });
      const res = await app.request("http://localhost/queries", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  test(
    "rejects ttl_seconds out of range",
    withEnv(openEnv, async () => {
      const app = buildWorkerApiApp({ queryService: testService });
