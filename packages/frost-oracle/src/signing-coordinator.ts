...
export async function coordinateSigning(
  ...
  for (const peer of nodeConfig.peers) {
    ...
    try {
      ...
      log.error(
        `Peer ${peer.signer_index} round1 error:`,
  ...
  if (Object.keys(commitments).length < nodeConfig.threshold) {
    log.error(
      `Only ${
        Object.keys(commitments).length
      }/${nodeConfig.threshold} commitments — below threshold`,
    );
    return null;
  }
  ...
  // This node's round 2
  const localR2 = await signRound2(
    keyPackageJson,
    localNonces,
    commitmentsJson,
    messageHex,
  ...
  for (const peer of participatingPeers) {
    try {
      ...
      log.error(
        `Peer ${peer.signer_index} round2 error:`,
        err instanceof Error ? err.message : err,
      );
    }
  }
  ...
  if (Object.keys(shares).length < nodeConfig.threshold) {
    log.error(
      ...
      }/${nodeConfig.threshold} shares — below threshold`,
    );
    return null;
  }
  ...
  const participated = [
    nodeConfig.signer_index,
    ...participatingPeers.map((p) => p.signer_index),
  ];
  return {
