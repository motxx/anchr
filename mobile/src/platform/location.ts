/**
 * Platform-abstracted geolocation.
 *
 * Native: expo-location
 * Web: navigator.geolocation (Geolocation API)
 */
...
function createWebProvider(): LocationProvider {
  return {
    async requestPermission() {
      ...
      // Geolocation API doesn't have a separate permission request;
      // permission is prompted on first getCurrentPosition call.
      // We can probe via Permissions API if available.
