...
- (フル E2E の場合) Stripe アカウント + Payment Link
...
Waiting for buyer to:
  1. Pay $70.00 via Stripe Payment Link: https://buy.stripe.com/test_xxxxx
...
Found order: query_xxxxx
  ...
  Bounty: 100000 sats
  Target URL: https://checkout.stripe.com/c/pay/{session_id}
  Conditions:
    - [contains] "succeeded" — Payment status must be succeeded
    - [contains] "$70.00" — Payment amount must be $70.00
    - [contains] "pi_demo_xxxxx" — Must match the specific payment intent
  Max attestation age: 600s

--- Step 2: Pay via Stripe ---
Open the Stripe Payment Link provided by the seller.
Complete the payment (credit card, Apple Pay, etc.).
Keep the receipt page open — you'll need it for the proof.

--- Step 3: Generate TLSNotary Proof ---
Use the TLSNotary browser extension to generate a proof:
  1. Open the Stripe receipt page in your browser
...
Example submission code:
  const proof = Bun.file("stripe-receipt.presentation.tlsn");
...
実際の TLSNotary proof 生成には Stripe での支払い + TLSNotary Extension が必要です。
...
4. 以下を入力:
   ...
   - Target URL: `https://checkout.stripe.com/c/pay/{session_id}`
   - Condition: ドロップダウンで **Contains** を選択 → `succeeded`
   - Bounty: `100000`
...
7. **Web Proof** バッジ付きのクエリカードが表示される (ドメイン `checkout.stripe.com` + `100000 sats`)
8. カードをクリックして展開
9. 以下が表示される:
   - **ターゲット URL**: `https://checkout.stripe.com/c/pay/{session_id}`
...
10. Seller が作成した Stripe Payment Link で支払いを完了する
11. Stripe の receipt ページを開いた状態で TLSNotary Extension を使って notarize
