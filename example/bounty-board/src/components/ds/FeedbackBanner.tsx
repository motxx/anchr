...
export function DSFeedbackBanner(
  { variant, message, children }: DSFeedbackBannerProps,
) {
  const v = VARIANT_CONFIG[variant];
  return (
    <View
      className={`${v.bg} border ${v.border} rounded-xl p-4 flex-row items-start gap-3`}
    >
      <Ionicons name={v.icon} size={18} color={v.iconColor} />
      <View className="flex-1">
        <DSText variant="body" weight="medium" color={v.textColor}>
          {message}
        </DSText>
        {children}
      </View>
