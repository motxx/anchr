...
export function loadAirdropBotShieldRuntimeConfig(): AirdropBotShieldRuntimeConfig {
  const port = Number(
    Deno.env.get("AIRDROP_BOT_SHIELD_PORT") ?? Deno.env.get("PORT") ?? "3000",
  );
  const dbPath = Deno.env.get("AIRDROP_BOT_SHIELD_DB_PATH")?.trim() ||
    ".airdrop-bot-shield/airdrop-bot-shield.db";
  const adminToken = Deno.env.get("AIRDROP_BOT_SHIELD_ADMIN_TOKEN")?.trim();
  const nullifierSecret =
    Deno.env.get("AIRDROP_BOT_SHIELD_NULLIFIER_SECRET")?.trim() ||
    "dev-nullifier-secret-change-before-mainnet";
  const settlement = loadSettlementConfig();
  const warnings = releaseConfigWarnings({
    dbPath,
    adminToken,
    nullifierSecret,
    settlement,
...
export function assertMainnetReleaseConfig(
  config: AirdropBotShieldRuntimeConfig,
): void {
  ...
  if (config.warnings.length > 0) {
    throw new Error(
      `Airdrop bot shield mainnet configuration is not release-ready: ${
...
function releaseConfigWarnings(opts: {
  ...
  if (!opts.adminToken || opts.adminToken.length < 32) {
    warnings.push(
      "AIRDROP_BOT_SHIELD_ADMIN_TOKEN must be set to at least 32 characters",
    );
  }
  if (
    !opts.nullifierSecret || opts.nullifierSecret.length < 32 ||
    opts.nullifierSecret.includes("dev-nullifier")
  ) {
    warnings.push(
      "AIRDROP_BOT_SHIELD_NULLIFIER_SECRET must be a non-default secret with at least 32 characters",
    );
  }
  ...
  if (!opts.settlement) {
    warnings.push(
      "AIRDROP_BOT_SHIELD_REQUESTER_REFUND_PUBKEY and AIRDROP_BOT_SHIELD_SOURCE_CASHU_TOKENS must configure Cashu HTLC settlement",
    );
  }
...
function loadSettlementConfig(): AirdropBotShieldSettlementConfig | undefined {
  const requesterRefundPubkey = Deno.env.get(
    "AIRDROP_BOT_SHIELD_REQUESTER_REFUND_PUBKEY",
  )?.trim();
  const sourceTokens = parseTokenList(
    Deno.env.get("AIRDROP_BOT_SHIELD_SOURCE_CASHU_TOKENS"),
  );
  if (!requesterRefundPubkey || sourceTokens.length === 0) return undefined;
  const locktimeRaw = Number(
    Deno.env.get("AIRDROP_BOT_SHIELD_HTLC_LOCKTIME_SECONDS") ?? "0",
  );
  const locktimeSeconds =
    Number.isInteger(locktimeRaw) && locktimeRaw > Math.floor(Date.now() / 1000)
      ? locktimeRaw
...
function parseTokenList(raw: string | undefined): string[] {
  if (!raw?.trim()) return [];
  return raw.split(/[\s,]+/).map((token) => token.trim()).filter((token) =>
    token.startsWith("cashu")
  );
}
