...
export default function QueryDetailScreen() {
  ...
  const handleTakePhoto = useCallback(async () => {
    ...
    if (photo) {
      ...
      if (Platform.OS !== "web") {
        const { deactivateKeepAwake } = await import("expo-keep-awake"); // allow-dynamic-import: native-only module, crashes if loaded on web
        deactivateKeepAwake();
      }
    }
  }, []);
  ...
  const handleOpenCamera = useCallback(async () => {
    ...
    const { activateKeepAwakeAsync } = await import("expo-keep-awake"); // allow-dynamic-import: native-only module, crashes if loaded on web
