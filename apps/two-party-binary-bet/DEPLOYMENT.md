...
MARKET_PORT=3001 \
deno run --config deno.json --allow-all apps/two-party-binary-bet/server.ts
```
...
deno run --allow-all apps/two-party-binary-bet/scripts/frost-dkg-bootstrap.ts \
  --threshold 2 --total 3 --output-dir .frost-market

# 2. Each signer node loads its own config. Boot the cluster.
deno run --allow-all apps/two-party-binary-bet/scripts/frost-oracle-cluster.ts
...
FROST_KEY_PASSPHRASE=$(openssl rand -hex 32) \
  deno run --allow-all apps/two-party-binary-bet/scripts/frost-prepare-secrets.ts --app anchr-market
...
- [ ] Run the screenshot script as a smoke test in CI:
      `deno run --allow-all apps/two-party-binary-bet/scripts/screenshots.ts`.
