...
| --- | --- | --- | --- |
| Paid app-owned HTTP route that creates or reads Anchr requests | External app adapter or tiny example | Yes, if the paid route only gates access to the app API | Good first target because it does not change Customer, Provider, Oracle, or Cashu HTLC semantics. |
| Paid MCP tool invocation for agents | External MCP adapter or tiny example | Yes, if the MCP adapter charges for tool access before calling SDK use cases | Useful for agent demos, but should stay adapter-local. |
