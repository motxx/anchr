# Land the target architecture in docs and add Unlock Condition to CONTEXT.md
...
Record the 2026-07-26 architecture decisions as the current target contract
...
1. `docs/architecture.md`: state the trust-boundary design — package
   ...
   shared trust base (wire types, build/parse, transition vocabulary);
   roles hold their own state models (Customer/Provider volatile per
   request, Oracle persistent); settlement schemes and proof schemas are
   swappable packages defined by obligations: async
   `prepareLock(request, mint)` (mint round-trips allowed — schemes like
   NUT-CTF register conditions before locking), `produceReleaseMaterial`
   (Oracle-side only), `verifyProviderBinding` (Provider-side), and pure
   deterministic `verify` for proof schemas (no ambient time, network, or
   ...
   contract; no migration narrative or measurement history.
...
- `predicate` is used in `specs/messaging.md` for the proof-schema
  ...
  vocabulary distinct.
- `docs/architecture.md` "Public Contract" / "Public Subpaths" sections
  predate the role-package split (0250) and should describe the target
  surface consistently with it; this issue may land before 0250 by
  describing the target, since pre-1.0 docs state the target contract.
...
- `docs/architecture.md` describes the trust-boundary package design and the
  plugin obligations as the current target, with no removed-design
  meta-commentary.
