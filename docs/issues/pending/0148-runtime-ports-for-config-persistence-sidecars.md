...
split this issue once 0117/0144/0150/0151 have decided which call sites
still exist.
...
3. **Sidecar execution** — schema verifiers and payment helpers spawn
   ...
   schema-bundle work in 0147 should make execution strategy a per-schema
...
4. **Server entrypoints** — `adapters/oracle-service/server.ts:150` calls
   ...
   entrypoint (final shape depends on 0151).
...
- `node:fs`/`node:os`/`node:path` imports in `proofs/` and `attachments/`
  fall away largely via 0144/0150; what remains follows the ports here.
...
- A browser-targeted import of the Customer/Provider core modules resolves
  without `Deno.*` or `node:*` references (gate automated in 0149).
