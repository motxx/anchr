...
- **Phase A — decide and delete** (shrinks every later phase):
  0144 (remove AI content check), 0150 (strip platform media tooling),
  0151 (canonical Oracle surface), 0117 (FROST complete-or-remove, existing).
- **Phase B — schema-extensibility core** (the centerpiece):
  0143 (decision: schema-owned verification) → 0145 (schema-scoped payloads)
  → 0146 (GPS out of shared core) → 0147 (runtime schema registration and
  reference adapters).
- **Phase C — portability**:
  0148 (runtime ports for config/persistence/sidecars) → 0149 (browser CI
  gate).
- **Phase D — privacy/P2P hardening** (mostly already tracked):
  0116, 0124, 0128 (existing), 0152 (FROST peer transport hook).
