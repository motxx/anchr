...
```bash
deno run --allow-all examples/supply-chain-proof/src/demo-coffee.ts
```
...
- [`examples/royalty-distribution/`](../royalty-distribution/) — sister verification-only chain example. Same pattern (recursive R/W/O along chain/graph edges, no Cashu settlement) but in the *fully digital* domain, where the physical-binding gap doesn't exist. Useful as the contrast — royalty fits the pattern more cleanly; supply-chain shows where the pattern hits its physical limit.
