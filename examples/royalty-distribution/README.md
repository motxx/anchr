...
- [`examples/supply-chain-proof/`](../supply-chain-proof/) — sister verification-only chain example. Same pattern (recursive R/W/O along chain edges, no Cashu settlement) but in the *physical* domain, where the photo-to-shipment binding gap is the hard problem. Royalty has no analogous binding gap.
