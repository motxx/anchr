/**
 * Quick Start: publish a Public Request Advertisement to a Nostr relay with
 * SDK-built events and read it back. One actor, one relay round trip.
 *
 * The advertisement carries only public discovery fields (ADR 0002): no
...
export interface QuickStartResult {
  /** The query id advertised on the relay. */
  queryId: string;
  /** Nostr event id of the published advertisement. */
  eventId: string;
  /** Relays that accepted the publish. */
  acceptedBy: string[];
  /** The advertisement read back from the relay and parsed. */
...
/**
 * Publish a minimal advertisement under a fresh ephemeral keypair, then
...
export async function runQuickStart(
  ...
  const echoedPromise = new Promise<QueryRequestPayload>(
    (resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(
          new QuickStartError(
            `relay did not echo the advertisement within ${timeoutMs}ms`,
          ),
        );
      }, timeoutMs);
  ...
  if (publishResult.successes.length === 0) {
    throw new QuickStartError(
      `no relay accepted the advertisement: ${
