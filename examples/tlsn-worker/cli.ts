 ...
 *
 * Usage:
 *   deno run --allow-all examples/tlsn-worker/cli.ts \
...
const worker = new AnchrWorker({
  serverUrl: args.server ?? Deno.env.get("ANCHR_SERVER_URL") ??
    "http://localhost:3000",
  verifierHost: args.verifier ?? Deno.env.get("TLSN_VERIFIER_HOST") ??
    "localhost:7047",
